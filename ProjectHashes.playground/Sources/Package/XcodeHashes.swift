...
public enum XcodeHashes {
    enum XcodeHashesError: Error {
        case noSuchFile
    }

    public static func hashStringFor(path: String) throws -> String {
        let url = URL(fileURLWithPath: path)

        guard FileManager.default.fileExists(atPath: url.path) else { throw XcodeHashesError.noSuchFile }
